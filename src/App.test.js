...
describe('isCatsGame', function () {
  let x = "x"
  let o = "o"
  it("returns TRUE because there are no available 0's on the board", function () {
    let gameBoard = [
      [x, o, x],
      [o, x, o],
      [o, x, o]
    ]
    assert.deepEqual(logic.isCatsGame(gameBoard), true)
  })
  it("returns FALSE because there are still available 0's on the board", function () {
    let gameBoard = [
      [x, o, x],
      [o, x, o],
      [o, x, 0]
    ]
    assert.deepEqual(logic.isCatsGame(gameBoard), false)
  })
})
